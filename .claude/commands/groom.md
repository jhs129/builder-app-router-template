...
If $ARGUMENTS is empty, query all tickets with `status = Grooming`. If a ticket key is provided (e.g. `<KEY>-8`), process only that ticket.
...
If $ARGUMENTS is empty, query the Grooming queue (includes tickets sent back from Requirements Review):
```
searchJiraIssuesUsingJql(
  cloudId="...",
  jql="project = <project-key> AND status in (Grooming, \"Requirements Feedback\") ORDER BY created ASC",
...
- If the ticket has enough context to proceed: transition to **Requirements Review**
  - If current status is **Grooming**: use the transition named **"Finalize Requirements Draft"**
  - If current status is **Requirements Feedback**: use the transition named **"Complete Requirements Feedback"**
...
- If multiple tickets are being processed, note progress as you go (e.g. "Processing <KEY>-8 (1 of 3)...")
