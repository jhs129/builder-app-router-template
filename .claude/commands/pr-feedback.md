...
Extract the Jira ticket key from the PR branch name (pattern: `<key>-NNN-*` → ticket key `<KEY>-NNN`):
```bash
# Branch names follow the convention: <key>-<number>-<slug>
# e.g., <key>-42-accordion-component → <KEY>-42
```

Get the Atlassian cloud ID and fetch the ticket:
```
...
If the ticket is in **In Review** status, transition it to **Code Review Feedback** using the **"Code Review Feedback"** transition to signal that feedback is actively being worked on:
```
getTransitionsForJiraIssue(cloudId="<cloud-id>", issueIdOrKey="<ticket-key>")
// Find the transition named "Code Review Feedback"
transitionJiraIssue(cloudId="<cloud-id>", issueIdOrKey="<ticket-key>", transitionId="<code-review-feedback-id>")
```

If the ticket is already in **Code Review Feedback**, skip the transition — it's already in the right state.
If no ticket key can be extracted from the branch name, skip this step and continue.
...
When the user asks you to push/commit the changes, after pushing successfully, transition the Jira ticket from **Code Review Feedback** back to **In Review** using the **"Complete Code Review Feedback"** transition:

```
getTransitionsForJiraIssue(cloudId="<cloud-id>", issueIdOrKey="<ticket-key>")
// Find the transition named "Complete Code Review Feedback"
transitionJiraIssue(cloudId="<cloud-id>", issueIdOrKey="<ticket-key>", transitionId="<complete-code-review-feedback-id>")
```

Add a comment to the Jira ticket summarizing the feedback that was addressed:
```
addCommentToJiraIssue(
  cloudId="<cloud-id>",
  issueIdOrKey="<ticket-key>",
  contentFormat="markdown",
  commentBody="**PR feedback addressed — moved back to In Review.**\n\n<bullet list of changes made>"
)
```
